...
Использует библиотеку SimPy для дискретно-событийного моделирования.
"""
...
class AssemblyLineModel:
    ...
    def is_work_time(self, current_time: float) -> bool:
        ...
        Для более точной модели можно добавить учет смен и выходных.
        """
        # Для упрощения считаем, что работа идет непрерывно
        # В реальной модели здесь была бы логика учета смен
        return True
    ...
    def run(self, until: Optional[float] = None):
        ...
        if until is None:
            # Запускаем до завершения всех процессов или до таймаута
            # Устанавливаем большой таймаут для безопасности (например, 1 год моделирования)
            until = 365 * 24 * 100  # 100 лет в часах - достаточно для любых экспериментов
